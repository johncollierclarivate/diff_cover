...
class DiffConditionCoverageViolations(object):
    """
    Class to capture violations generated by a particular diff
    """
    def __init__(self, violations, measured_lines, diff_lines):
        violations = list(filter(lambda x: x.line in diff_lines, violations))

        self.total_conditions = sum(violation.total
                                    for violation in violations)

        violations = list(filter(lambda x: x.missed > 0 and x.line in diff_lines, violations))
...
class BaseReportGenerator:
    ...
    def __init__(self, violations_reporter, diff_reporter):
        ...
        self._diff_condition_coverage_violations_dict = None
    ...
    def percent_conditions_covered(self, src_path):
        """
        Return a float percent of lines covered for the source
        in `src_path`.
        ...
        If we have no coverage information for `src_path`, returns None
        """
        diff_violations = self._diff_condition_coverage_violations().get(src_path)

        if diff_violations is None:
            return 100

        num_conditions = diff_violations.total_conditions
        num_conditions_missing = sum(violation.missed for violation in diff_violations.violations)
        # Protect against a divide by zero
        if num_conditions > 0:
            return 100 - float(num_conditions_missing) / num_conditions * 100
        else:
            return 100
    ...
    def condition_coverage_violation_lines(self, src_path):
        """
        Return a list of lines in violation (integers)
        in `src_path` that were changed.
    ...
    def total_num_conditions(self):
        """
        Return the total number of lines in the diff for
        which we have coverage info.
        """

        return sum(summary.total_conditions for summary
                    in self._diff_condition_coverage_violations().values())
    ...
    def total_num_condition_violations(self):
        """
        Returns the total number of lines in the diff
        that are in violation.
        """

        return sum([sum(violation.missed for violation in summary.violations) for summary
                    in self._diff_condition_coverage_violations().values()])

    def total_percent_conditions_covered(self):
        """
        Returns the float percent of lines in the diff that are covered.
        (only counting lines for which we have coverage info).
        """
        total_conditions = self.total_num_conditions()

        if total_conditions > 0:
            num_covered = total_conditions - self.total_num_condition_violations()
            return int(float(num_covered) / total_conditions * 100)
    ...
    def _diff_condition_coverage_violations(self):
        """
        Returns a dictionary of the form:
        ...
        To make this efficient, we cache and reuse the result.
        """
        if not self._diff_condition_coverage_violations_dict:
            self._diff_condition_coverage_violations_dict = {
                src_path: DiffConditionCoverageViolations(
                    self._violations.violations_condition_coverage(src_path),
                    self._violations.measured_lines(src_path),
                    self._diff.lines_changed(src_path),
                )
                for src_path in self._diff.src_paths_changed()
            }
        return self._diff_condition_coverage_violations_dict
    ...
    def report_dict(self):
        ...
        return {
            ...
            "total_num_conditions": self.total_num_conditions(),
            "total_num_missed_conditions": self.total_num_condition_violations(),
            "total_percent_conditions_covered": self.total_percent_conditions_covered(),
        }
    ...
    def _src_path_stats(self, src_path):
        ...
        # Find condition coverage violation lines
        condition_coverage_violation_lines = self.condition_coverage_violation_lines(src_path)
        condition_coverage_violations = sorted(self._diff_condition_coverage_violations()[src_path].violations)
        condition_coverage_messages = list(map(lambda violation: violation.message(), condition_coverage_violations))

        return {
            "percent_covered": self.percent_covered(src_path),
            "violation_lines": violation_lines,
            "violations": violations,
            "conditions_percent_covered": self.percent_conditions_covered(src_path),
            "condition_coverage_messages": condition_coverage_messages,
            "condition_coverage_violation_lines": condition_coverage_violation_lines,
            "condition_coverage_violations": condition_coverage_violations
        }
...
class TemplateReportGenerator(BaseReportGenerator):
    ...
    def _src_path_stats(self, src_path):
        ...
        stats.update({
            ...
            "condition_coverage_violation_lines": TemplateReportGenerator.combine_adjacent_lines(stats['condition_coverage_violation_lines']),
        })
